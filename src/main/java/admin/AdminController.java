...
public class AdminController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(level > 0) {
			...
			command.execute(request, response);
			viewPage += "/adminBoardContent.jsp";
		}
		
		else if(com.equals("/AdminAskBoardList")) {
			command = new AdminAskBoardListCommand();
			command.execute(request, response);
			viewPage += "/adminAskBoardList.jsp";
		}
		else if(com.equals("/AdminAskBoardInput")) {
			viewPage += "/adminAskBoardInput.jsp";
		}
		else if(com.equals("/AdminAskBoardInputOk")) {
			command = new AdminAskBoardInputOkCommand();
			command.execute(request, response);
			viewPage = "/include/message.jsp";
		}
		else if(com.equals("/AdminAskBoardContent")) {
			command = new AdminAskBoardContentCommand();
			command.execute(request, response);
			viewPage += "/adminAskBoardContent.jsp";
		}
		
		else if(com.equals("/AdminExhibitionList")) {
			command = new AdminExhibitionListCommand();
			command.execute(request, response);
			viewPage += "/adminExhibitionList.jsp";
		}
		else if(com.equals("/AdminExhibitionInput")) {
			viewPage += "/adminExhibitionInput.jsp";
		}
		else if(com.equals("/AdminExhibitionInpuOk")) {
			command = new AdminExhibitionInpuOkCommand();
			command.execute(request, response);
			viewPage = "/include/message.jsp";
		}
		...
		else if(com.equals("/AdminExhibitionPartChange")) {
			command = new AdminExhibitionPartChangeCommand();
			command.execute(request, response);
			return;
		}
